...
function blocklist_get_config($engine)
{
	//Handled in the class
	return true;
}

function blocklist_blocklist_add($fc)
{
	//Handled in the class
	return true;
}

function blocklist_blocklist_remove($fc)
{
	//Handled in the class
	return true;
}

function blocklist_blocklist_last($fc)
{
	//Handled in the class
	return true;
}
//not sure how to make this BMO
function blocklist_hookGet_config($engine)
{
	global $ext;
	switch ($engine) {
		case "asterisk":
			...
			if (is_array($didlist)) {
				foreach ($didlist as $item) {
					...
					$ext->splice($context, $exten, 'did', new ext_gosub('1', 's', 'app-blocklist-check'));
...
function blocklist_list()
{
	return FreePBX::Blocklist()->getBlocklist();
}

function blocklist_del($number)
{
	return FreePBX::Blocklist()->numberDel($number);
}

function blocklist_add($post)
{
	return FreePBX::Blocklist()->numberAdd($post);
}

// ensures post vars is valid
function blocklist_chk($post)
{
	return FreePBX::Blocklist()->checkPost($post);
}
