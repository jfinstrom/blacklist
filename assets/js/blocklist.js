...
$("#submitnumber").on("click", function() {
  ...
  $.post(
    "ajax.php?module=blocklist&command=add",
    {
      action: "add",
      oldval: oldv,
      number: num,
    ...
    function(data, status) {
      ...
      if (data.status) {
        if (oldv.length > 0) {
          ...
          alert(sprintf(_("Added %s to the blocklist."), num));
        }
...
$(document).on("click", '[id^="del"]', function() {
  ...
  if (confirm(_("Are you sure you want to delete this item?"))) {
    $.post("ajax.php?module=blocklist&command=del", {
      action: "delete",
      number: num,
    }).done(function() {
...
$(document).on("click", '[id^="report"]', function() {
  var num = $(this).data("number");
  $.post("ajax.php?module=blocklist&command=calllog", { number: num }, function(
    data,
    status
  ) {
    console.log(data);
...
$("#Upload").on("click", function() {
  var file = document.getElementById("blocklistfile");
  var formData = new FormData();
  formData.append("blocklistfile", file.files[0]);
  var xhr = new XMLHttpRequest();
  xhr.open("POST", "config.php?display=blocklist&action=import", true);
  xhr.send(formData);
...
$("#blkDelete").on("click", function(e) {
  ...
  $.post("ajax.php?module=blocklist&command=bulkdelete", {
